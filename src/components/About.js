      ...
      <div id="about" className="flex justify-between cursor-pointer">
        ...
        <div className="w-full flex flex-col items-center text-center mt-8 cursor-normal space-y-2"  >
            <img ref={containerRef2} src="/profile.png" id="profile-pic" alt="Profile" className={`border-2 bg-gray-300 dark:bg-gray-800 ${isVisible2 && 'animate-fade-in-down'} opacity-0 mb-4 border-gray-900 dark:border-gray-100 w-48 h-48 rounded-full shadow-lg`} style={{ "--delay": 0.2 + "s" }}  /> 
              <p ref={containerRef3} className={`${isVisible3 && 'animate-fade-in'} opacity-0`} style={{ "--delay": 1.2 + "s" }}>
              Hello 🙋‍♂️ I'm <span className='font-medium'>Manos</span>, a Computer Engineer currently based in Heraklion, Greece 🇬🇷.
              </p>
              <p ref={containerRef4} className={`${isVisible4 && 'animate-fade-in'} opacity-0`} style={{ "--delay": 1.6 + "s" }}>
              ...
              <p ref={containerRef5} className={`${isVisible5 && 'animate-fade-in'} opacity-0`} style={{ "--delay": 2 + "s" }}>
              ...
              <p ref={containerRef6} className={`${isVisible6 && 'animate-fade-in'} opacity-0`} style={{ "--delay": 2.4   + "s" }}>I'm eager to contribute to real-world projects and make a meaningful impact, so feel free to <a className="hover:underline" href="mailto:manosmin@hotmail.com">contact me</a> for related opportunities 🌍.</p>
        </div>
      </div>
  );
};
